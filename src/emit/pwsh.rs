...
impl PwshEmitter {
    fn node(&self, n: &Node, d: usize) -> Vec<String> {
        match n {
            Node::Set { key, val } =>
                vec![self.indent(format!("$env:{} = \"{}\"", key, val), d)],
            ...
            Node::Inject { cmd, args } => {
                ...
                let call = if a.trim().is_empty() {
                    ...
                    format!("Invoke-Expression (& {} {})", cmd, a.trim())
                };
...
mod tests {
    ...
    #[test]
    fn path_prepend() {
        let out = render(&[Node::Path { dir: "C:\\tools".into(), prepend: true }]);
        assert_eq!(out, "$env:PATH = \"C:\\tools;$env:PATH\"");
    }

    #[test]
    fn path_append() {
        let out = render(&[Node::Path { dir: "C:\\opt".into(), prepend: false }]);
